...
const yearData = ref<
  { year: string; men: number; women: number; undefined: number }[]
>([]);
const filteredYearData = ref<
  { year: string; men: number; women: number; undefined: number }[]
>([]);
    ...
    const option: echarts.EChartsOption = {
      ...
      series: [
        ...
        },
        {
          name: "Total",
          type: "line",
          data: filteredYearData.value.map(
            (item) => item.men + item.women + item.undefined
          ),
          itemStyle: {
            color: "#E74C3C",
          },
...
watch(selectedInterval, (newInterval) => {
  if (newInterval === "all") {
    ...
    filteredYearData.value = yearData.value.filter(
      (item) =>
        parseInt(item.year) >= startYear && parseInt(item.year) <= endYear
    );
  }
